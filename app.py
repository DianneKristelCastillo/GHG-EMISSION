...
def emu_dashboard():
    ...
    try:
        ...
        # Queries for data
        queries = [
            ("SELECT month, kg_co2_per_kwh FROM electricity_consumption WHERE year = %s ORDER BY month ASC", electricity_data, "electricity"),
            ("SELECT MONTHNAME(date) AS month, total_emission FROM fuel_emissions WHERE YEAR(date) = %s ORDER BY MONTH(date) ASC", fuel_data, "fuel"),
            ("SELECT Month, SUM(GHGEmissionKGCO2e) AS total_emission FROM tblsolidwastesegregated WHERE Year = %s GROUP BY Month ORDER BY FIELD(Month, 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December')", waste_segregated_data, "waste_segregated"),
            ("SELECT Month, GHGEmissionKGCO2e FROM tblsolidwasteunsegregated WHERE Year = %s ORDER BY FIELD(Month, 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December')", waste_unsegregated_data, "waste_unsegregated"),
            ("SELECT MONTHNAME(Date) AS month, SUM(FactorKGCO2e) AS total_emission FROM tblwater WHERE YEAR(Date) = %s GROUP BY MONTHNAME(Date) ORDER BY FIELD(MONTHNAME(Date), 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December')", water_data, "water"),
        ]

        # Executing queries without campus filter
        ...
        # Treated Water Data without campus filter
        treated_water_query = """
            SELECT Month, SUM(FactorKGCO2e) AS total_emission
            FROM tbltreatedwater
            WHERE YEAR(CURDATE()) = %s
            GROUP BY Month
            ORDER BY FIELD(Month, 'January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December')
        """
        cursor.execute(treated_water_query, [selected_year])
        for row in cursor.fetchall():
    ...
    def forecast_prophet(data, periods, freq='M', smoothing_factor=0.2):
        ...
        try:
            # Create a DataFrame with dates and emissions
            ...
            model = Prophet(yearly_seasonality=True, daily_seasonality=False)  # Weekly seasonality turned off
            model.fit(df)
...
def sdo_dashboard():
    ...
    selected_year = int(request.args.get('year', datetime.now().year))  # Default to the current year if not provided
    current_year = datetime.now().year

    # Initialize data for LPG, Food Waste, Accommodation, and Flight
    electricity_data = [0] * 12
    fuel_data = [0] * 12
    waste_segregated_data = [0] * 12
    waste_unsegregated_data = [0] * 12
    ...
    try:
        ...
        # Execute queries for all the categories
        for query, data_list, category in queries:
            cursor.execute(query, (selected_year,))
            for row in cursor.fetchall():
                month_index = month_to_index.get(row.get('month') or row.get('Month'), -1)
    ...
    accommodation_forecast, accommodation_r2, accommodation_mse, accommodation_mae = forecast_prophet(
        accommodation_data, periods=forecast_periods_yearly, freq='Y'
    )
    flight_forecast, flight_r2, flight_mse, flight_mae = forecast_prophet(
        flight_data, periods=forecast_periods_yearly, freq='Y'
    )
    ...
    # Month labels
    labels = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'] + ['Future Month 1', 'Future Month 2']
...
def sdo_waste_segregation_report():
    ...
    month_filter = request.args.get("month", None)  # Get the selected month filter
    # Pagination setup
    per_page = 20  # Records per page
    current_page = request.args.get('page', 1, type=int)
...
def procurement_dashboard():
    ...
    # Extract parameters
    selected_campus = request.args.get('campus', 'all')  # Default to "all" campuses
    selected_year = int(request.args.get('year', datetime.now().year))
    current_year = datetime.now().year

    # Initialize data for Food Waste and LPG
    food_waste_data = [0] * 12
    lpg_data = [0] * 12
    ...
    # Print R² scores and other metrics for debugging/validation
    print("\nFood Waste Forecast Metrics:")
    print(f"  R² Score: {food_waste_r2:.2f}")
    print(f"  MSE: {food_waste_mse:.2f}")
    print(f"  MAE: {food_waste_mae:.2f}")
    ...
    try:
        ...
        logging.error(f"Error in procurement dashboard: {e}")
        flash(f"An error occurred: {e}", 'danger')
        # Redirect to the home page (or another page you prefer)
        return redirect(url_for('home'))
